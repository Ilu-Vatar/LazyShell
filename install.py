...
try:
    ...
    if whattodo == '1':
        ...
        os.system('apt update && apt install python3 && apt install net-tools && apt install nmap && apt install python-pip && pip install terminaltables && apt install git && apt install -f && echo Finished')
        sys.exit()
    elif whattodo == '3':
        sys.exit('Exiting')
    else:
...
def main():
    ...
    os.system("""mkdir /opt/lazyshell && cp lazyshell.py /opt/lazyshell/lazyshell.py && cp banner.py /opt/lazyshell/banner.py && cp -r data /opt/lazyshell/data && cp lazy.sh /usr/bin/lazyshell && chmod +x /usr/bin/lazyshell && echo "LazyShell has been successfully installed. Execute 'lazyshell' in your terminal"   """)
